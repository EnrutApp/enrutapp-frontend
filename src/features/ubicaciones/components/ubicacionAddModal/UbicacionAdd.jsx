...
const UbicacionAdd = ({
  ...
  const mostrarMapa = direccionFijada || ubicacionParaMapa;
  const deberiaMostrarEstadoVacio =
    !nombre.trim() &&
    !direccion.trim() &&
    !latitud &&
    !longitud &&
  ...
  useEffect(() => {
    if (coordenadasEncontradas && latitud && longitud && !direccionFijada) {
      const timer = setTimeout(() => {
        setDireccionFijada(true);
      }, 2000);
      return () => clearTimeout(timer);
    }
  }, [coordenadasEncontradas, latitud, longitud, direccionFijada]);

  useEffect(() => {
    if (!deberiaMostrarEstadoVacio && isEstadoVacioVisible) {
      const timer = setTimeout(() => {
        setIsEstadoVacioVisible(false);
      }, 300);
      return () => clearTimeout(timer);
    } else if (deberiaMostrarEstadoVacio && !isEstadoVacioVisible) {
      setIsEstadoVacioVisible(true);
    }
  }, [deberiaMostrarEstadoVacio, isEstadoVacioVisible]);
        ...
        <div className="flex gap-0 h-[90vh]">
              ...
              >
                ...
                <div>
                  ...
                  {isMensajeInicialVisible && (
                    <div
                      className={`content-box-outline-2-small mt-5 transition-opacity duration-300 ${deberiaMostrarMensajeInicial
                          ? 'opacity-100'
                          : 'opacity-0'
                        }`}
                    >
          ...
          <div className="w-[58%] relative overflow-hidden rounded-r-3xl">
            <MapBoxMap
              ...
              allowClickSelection={direccionFijada}
              onMapClick={handleMapClick}
              showDefaultMap={!ubicacionParaMapa && !direccionFijada}
              initialCenter={mapCenter}
              initialZoom={mapZoom}
              enable3D={true}
              mapStyle="dark"
            />
            {isEstadoVacioVisible && deberiaMostrarEstadoVacio && (
              <div className="absolute inset-0 flex items-center justify-center bg-background/50 rounded-r-3xl backdrop-blur-sm z-20 transition-opacity duration-300 pointer-events-none">
                <div className="content-box-outline-5-small max-w-md mx-auto pointer-events-auto">
                  <div className="flex flex-col items-center p-6">
                    ...
                    <div className="flex flex-col items-center gap-2 text-center justify-center">
                      <h1 className="text-h5 text-primary font-semibold mb-1">
                        Comienza agregando una ubicación
                      </h1>
                      <p className="text-sm text-secondary font-normal">
                        Completa el formulario a la izquierda para agregar una
                        nueva ubicación. Una vez ingreses la dirección, podrás
                        verla en el mapa.
                      </p>
                    </div>
                  </div>
            ...
            {direccionFijada && (
              <div className="absolute bottom-6 left-6 right-6 bg-background rounded-lg p-4 z-10 shadow-lg">
                <div className="flex items-center gap-3">
                  <div className="bg-fill rounded-full p-2 w-10 h-10 flex items-center justify-center">
