...
const Reservas = () => {
  ...
  const filteredReservas = allReservas
    .filter(reserva => {
      const matchesSearch =
        ...
        (reserva.pasajeros &&
          ...
          reserva.pasajeros.some(
            p =>
              typeof p === 'string' &&
              p.toLowerCase().includes(searchQuery.toLowerCase())
          )) ||
        (reserva.origen &&
          reserva.origen.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (reserva.destino &&
          reserva.destino.toLowerCase().includes(searchQuery.toLowerCase()));
      const matchesFilter =
        filterEstado === 'Todos' || reserva.estado === filterEstado;
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'Fecha') {
        ...
        comparison = (a.nombreCliente || '').localeCompare(
          b.nombreCliente || ''
        );
      }
  ...
  const handleEditConfirm = async updatedData => {
    try {
      setError(null);
      if (reservaToEdit.id) {
        const response = await reservaService.updateReserva(
          reservaToEdit.id,
          updatedData
        );
        if (response.success) {
          setAllReservas(prev =>
            prev.map(r =>
              r.id === reservaToEdit.id ? { ...r, ...updatedData } : r
            )
          );
        } else {
    ...
    return (
      <section className="flex items-center justify-center h-96">
        <div className="text-center">
          <md-icon className="text-4xl text-primary animate-spin">
            settings
          </md-icon>
          ...
          <div className="list-enter">
            ...
            <div className="flex justify-between items-center mt-6 mb-4">
              <span className="text-sm text-secondary">
                ...
                {Math.min(startIndex + 5, totalItems)} de {totalItems} reservas
              </span>
              {showPagination && (
            ...
            <div className="mt-3">
              {currentReservas.length > 0 ? (
                  ...
                  {currentReservas.map((reserva, index) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1 whitespace-nowrap ml-2 ${
                      ...
                      <div className="mb-3 p-2 bg-fill border border-outline rounded">
                        ...
                        <div className="flex justify-center my-2">
                          <md-icon className="text-sm text-outline">
                            arrow_downward
                          </md-icon>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="space-y-1">
                          ...
                          {reserva.pasajeros?.length > 2 && (
                            <p className="text-xs text-secondary italic">
                              +{reserva.pasajeros.length - 2} más
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="border-t border-outline pt-3 mb-3">
                        {reserva.nombreConductor ? (
                          <div className="mb-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-secondary bg-outline px-2 py-1 rounded">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-primary">
                            ${reserva.precio?.toLocaleString() || '0'}
                          </p>
                        </div>
                      </div>
                ...
                <div className="text-center py-12">
                  <md-icon className="text-6xl text-outline mb-4 opacity-50">
                    calendar_today
                  </md-icon>
                  ...
                  <p className="text-xs text-outline mt-1">
                    Las reservas que crees aparecerán aquí
                  </p>
                  <button
                    onClick={() => setIsAddModalOpen(true)}
