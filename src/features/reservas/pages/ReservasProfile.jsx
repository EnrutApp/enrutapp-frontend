    ...
    return (
        <div className={`flex flex-col gap-4 overflow-auto ${isClosing ? 'profile-exit' : 'profile-enter'}`}
            ...
            <div className='bg-primary text-on-primary content-box-small'>
                <h1 className='h3 text-on-primary'>{reserva.origen} - {reserva.destino}</h1>
                <span className='subtitle1 font-medium text-on-primary'>Fecha: {reserva.fecha}</span>
                <span className='subtitle1 font-medium text-on-primary'>Hora: {reserva.hora}</span>
            </div>
            ...
            <div className="flex flex-col gap-3 flex-1">
                <div className="flex flex-col">
                    <div className="content-box-outline-3-small">
                        ...
                        <div className='flex mt-1'>
                            <button className={`btn font-medium btn-lg flex items-center ${
                                reserva.estado === 'Pendiente' ? 'btn-yellow' : 
                                reserva.estado === 'Completada' ? 'btn-green' : 
                                'btn-red'
                            }`}>
                ...
                <div className="flex flex-col">
                    <div className="content-box-outline-3-small">
                        <span className="subtitle1 text-primary font-light">Información del viaje</span>
                ...
                <div className="flex flex-col">
                    <div className="content-box-outline-3-small">
                        <span className="subtitle1 text-primary font-light">Cliente y pasajeros</span>
                        <span className='subtitle1 text-secondary mt-1'>
                            Cliente: {reserva.nombreCliente || 'N/A'}
                        </span>
                        <span className='subtitle1 text-secondary mt-1'>
                            Total de pasajeros: {Array.isArray(reserva.pasajeros) ? reserva.pasajeros.length : 0}
                        </span>
                        {Array.isArray(reserva.pasajeros) && reserva.pasajeros.length > 0 ? (
                            <div className="mt-3 space-y-2">
                                {reserva.pasajeros.map((pasajero, idx) => (
                                    <div key={idx} className="p-2 bg-fill rounded border border-outline">
                                        <span className='text-sm text-primary'>
                                            {idx + 1}. {typeof pasajero === 'string' ? pasajero : pasajero.nombre}
                                        </span>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="flex flex-col">
                    <div className="content-box-outline-3-small">
                        ...
                        <span className='subtitle1 text-secondary mt-1'>
                            Precio: ${reserva.precio ? parseFloat(reserva.precio).toFixed(2) : '0.00'}
                        </span>
                    </div>
                </div>
