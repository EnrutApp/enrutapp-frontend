  ...
  const onSubmit = async data => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(
          response.error || response.message || 'Error al completar perfil'
        );
      }
    } catch (err) {
      setError(
              ...
              <div className="flex flex-col gap-8 list-enter min-h-[300px] justify-center">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-secondary text-sm">
                    Para continuar como cliente, necesitamos que completes tu
                    información básica.
                  </p>
                </div>
                      ...
                      render={({ field }) => (
                        <select
                          ...
                          className={`select-filter w-full px-4 input bg-fill border rounded-lg text-primary focus:outline-none focus:border-primary transition-colors ${
                            errors.tipoDoc ? 'border-red-500' : 'border-border'
                          }`}
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <select
                          {...field}
                          className={`select-filter w-full px-4 input bg-fill border rounded-lg text-primary focus:outline-none focus:border-primary transition-colors ${
                            errors.idCiudad ? 'border-red-500' : 'border-border'
                          }`}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const numDocumento = String(
                        getValues('numDocumento') ?? ''
                      ).trim();

                      const currentNumDocumento = String(
                        user?.numDocumento ?? ''
                      ).trim();
                      if (
                        currentNumDocumento &&
                        currentNumDocumento === numDocumento
                      ) {
                        setStep(3);
                        return;
                      }
                      ...
                      try {
                        ...
                        setError(
                          'Error validando el documento. Inténtalo de nuevo.'
                        );
                      } finally {
