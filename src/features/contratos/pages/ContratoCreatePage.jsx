...
const ContratoCreatePage = () => {
  ...
  const validateFechaRealtime = value => {
    ...
    if (selectedDate < today) {
      return 'No puede ser pasada ni mayor a 1 año';
    }

    // No puede ser mayor a 1 año desde hoy
  ...
  const handleFieldChange = (fieldName, value) => {
    if (fieldName === 'placa') {
      // Filtrar caracteres inválidos en tiempo real
      const filtered = validatePlacaRealtime(value);
      setForm(prev => ({ ...prev, [fieldName]: filtered }));
    } else if (fieldName === 'fechaOrigen' || fieldName === 'fechaDestino') {
      // Validar fecha en tiempo real
      const error = validateFechaRealtime(value);
  ...
  const prefillFromTurno = async turno => {
    ...
    const tipoVehiculo =
      turno?.vehiculo?.tipoVehiculo?.nombreTipoVehiculo || '';
    const capacidadPasajeros = String(
      turno?.vehiculo?.capacidadPasajeros ?? ''
    );
      ...
      const matched = pasajerosFromPasajes
        .map(p => {
          const cliente = clientes.find(
            c => String(c.document) === String(p.documento)
          );
  ...
  const validate = () => {
    ...
    if (form.placa.trim().length < 1) return 'La placa es obligatoria';
    if (!form.origen.trim() || !form.destino.trim())
      return 'Origen y destino son obligatorios';
    if (!form.fechaOrigen || !form.fechaDestino)
      return 'Las fechas son obligatorias';

    // Validar que no haya errores en los campos
    if (fieldErrors.placa) return fieldErrors.placa;
    if (fieldErrors.fechaOrigen) return fieldErrors.fechaOrigen;
    if (fieldErrors.fechaDestino) return fieldErrors.fechaDestino;
    ...
    <section className="list-enter">
      ...
      {loading ? (
        ...
        <form
          ...
          className="content-box-outline-2-small p-4"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                value={form.placa}
                onChange={e =>
                  handleFieldChange('placa', e.target.value.toUpperCase())
                }
                className={`w-full px-4 py-3 input bg-fill border rounded-lg text-primary placeholder-text-secondary focus:outline-none transition-colors ${
                  fieldErrors.placa
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={e =>
                  updateField('capacidadPasajeros', e.target.value)
                }
                className="w-full px-4 py-3 input bg-fill border rounded-lg text-primary placeholder-text-secondary focus:outline-none focus:border-primary transition-colors border-border"
                placeholder="0"
              />
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={e => handleFieldChange('fechaOrigen', e.target.value)}
                className={`w-full px-4 py-3 input bg-fill border rounded-lg text-primary placeholder-text-secondary focus:outline-none transition-colors ${
                  fieldErrors.fechaOrigen
                    ? 'border-error-500 focus:border-error-500'
                    : 'border-border focus:border-primary'
                }`}
              />
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={e =>
                  handleFieldChange('fechaDestino', e.target.value)
                }
                className={`w-full px-4 py-3 input bg-fill border rounded-lg text-primary placeholder-text-secondary focus:outline-none transition-colors ${
                  fieldErrors.fechaDestino
                    ? 'border-error-500 focus:border-error-500'
                    : 'border-border focus:border-primary'
                }`}
              />
          ...
          <div className="mt-6">
            <div className="flex justify-between items-center">
              ...
              <md-filled-button
                ...
                className="btn-add px-5"
                onClick={addPasajero}
                disabled={saving || pasajeros.length >= 50}
              >
                <md-icon slot="icon" className="text-sm text-on-primary">
                  person_add
                </md-icon>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {pasajeros.map((p, idx) => {
                    ...
                    const isAtMaxCapacity = pasajeros.length >= 50;
                    return (
                      <tr key={idx} className="border-t border-border">
                        ...
                        <td className="py-2 pr-3">
                          <md-filled-button
                            ...
                            disabled={saving || isAtMaxCapacity}
                          >
                            Quitar
                          </md-filled-button>
                        </td>
            ...
            <p className="text-xs text-secondary mt-2">
              El PDF se genera con el template de 3 páginas y una lista simple
              de pasajeros (ajustable).
            </p>
          </div>
        </form>
      )}
