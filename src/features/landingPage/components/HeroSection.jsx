  ...
  ({
    ...
    const handleDestinoChange = value => {
      ...
      if (
        !error &&
        formData.origen &&
        value.toLowerCase() === formData.origen.toLowerCase()
      ) {
    ...
    const handleDestinoSelect = city => {
      ...
      if (
        formData.origen &&
        city.city.toLowerCase() === formData.origen.toLowerCase()
      ) {
        error = 'Origen y destino no pueden ser la misma ciudad';
      }
    ...
    const handleDateChange = e => {
      ...
      if (name === 'fecha' && today) {
        if (value < today) {
          setErrors(prev => ({
            ...prev,
            fecha: 'No puedes seleccionar una fecha en el pasado',
          }));
          return;
        }
      }
    ...
    return (
      <section
        id="inicio"
        className="relative min-h-[800px] flex items-center px-4 md:px-8 pt-20 overflow-hidden"
      >
        {/* Background */}
        <div className="absolute inset-0 z-0">
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(to right, rgba(3, 7, 18, 0.92), rgba(3, 7, 18, 0.65), rgba(3, 7, 18, 0.25))',
            }}
          />
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto py-16">
          <div className="grid lg:grid-cols-12 gap-12 items-center">
                  ...
                  <div className="flex flex-col">
                      ...
                      <div className="flex bg-fill rounded-lg p-1 border border-border">
                        {['Hoy', 'Mañana'].map(tipo => (
                          <button
                            ...
                            className={`px-3 py-1 rounded-md text-caption font-bold transition-all duration-200 ${
                    ...
                    <div className="relative group/input">
                      <input
                        ...
                        className={`w-full px-4 py-3 input bg-fill border rounded-lg text-primary placeholder-text-secondary date-secondary focus:outline-none focus:border-primary transition-colors duration-200 ${
                          errors.fecha ? 'border-red-500' : 'border-border'
                        }`}
                      />
                    </div>
                    {errors.fecha && (
            ...
            <div
              ref={contentRef}
              className="lg:col-span-7 text-primary space-y-8 lg:pl-16 lp-fade-up lp-delay-200"
            >
              <div className="space-y-5">
                ...
                <h1 className="text-h1 font-bold leading-tight tracking-tight text-white drop-shadow-sm">
                  Viajar está a un
                  <br />
                  <span className="text-white">Clic de distancia</span>
                </h1>

                <p className="text-h5 text-white/75 font-regular max-w-xl leading-relaxed">
                  Compra tus tiquetes de forma rápida, segura y sin
                  complicaciones
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                {[
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-bold text-white text-body1">
                        {feature.title}
                      </h4>
