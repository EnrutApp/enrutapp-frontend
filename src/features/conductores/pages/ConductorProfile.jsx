...
const ConductorProfile = ({
  ...
  const handleProfileEdit = async (id, data) => {
    try {
      let response;
      if (id) {
        response = await conductorService.updateConductor(id, data);
      } else {
        response = await conductorService.createConductor({
          idUsuario: conductorData.idUsuario,
          ...data,
        });
      }
  ...
  const handleCategoriaClick = async () => {
    if (!conductorData.idConductor) {
      alert(
        'Este usuario aún no tiene un perfil de conductor creado. Por favor usa el botón "Editar datos" para completar su información (Licencia y Vencimiento) primero.'
      );
      return;
    }
  ...
  const handleCategoriaConfirm = async () => {
    ...
    try {
      const response = await conductorService.updateConductor(
  ...
  const handleContactoConfirm = async () => {
    ...
    try {
      ...
      const response = await conductorService.getConductorById(
        conductorData.idConductor
      );
      const updatedConductor = response.data || response;
      setFullConductor(updatedConductor);
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col">
          <div className="content-box-outline-3-small">
            ...
            <div className="flex mt-1">
              ...
              >
                {conductorData.estado || usuarioData.estado
                  ? 'Activo'
                  : 'Inactivo'}
              </button>
            </div>
      ...
      <EditConductorModal
        ...
        onUpdateConductor={handleProfileEdit}
      />
