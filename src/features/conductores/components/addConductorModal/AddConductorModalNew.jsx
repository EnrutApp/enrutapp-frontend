...
export default function AddConductorModalNew({ isOpen, onClose, onSuccess }) {
    ...
    const cargarDatosIniciales = async () => {
        try {
            ...
            const todosUsuarios = responseUsuarios.data?.data || responseUsuarios.data || [];
            ...
            const usuariosSinPerfil = todosUsuarios.filter(u => {
                ...
                const tienePerfil = conductoresConPerfil.some(c =>
                    c.idUsuario === u.idUsuario && c.numeroLicencia
                );
    ...
    const handleCrearNuevoConductor = async () => {
        try {
            ...
            if (error.response?.data?.message) {
                ...
                if (backendMessage.includes('Unique constraint failed') && backendMessage.includes('correo')) {
                    errorMessage = 'El correo electrónico ya está registrado. Por favor, usa otro correo.';
                }
                // Detectar error de documento duplicado
                else if (backendMessage.includes('Unique constraint failed') && backendMessage.includes('numDocumento')) {
            ...
            // Si el error es en el paso de invitación, volver al paso 3
            if (completionMode === 'invite') {
                setStep(3);
                setCompletionMode(null);
            }
    ...
    const validarPaso1 = async () => {
        try {
            ...
            const correoExiste = todosUsuarios.some(u =>
            ...
            const documentoExiste = todosUsuarios.some(u =>
                u.numDocumento === nuevoUsuario.numDocumento && u.tipoDoc === nuevoUsuario.tipoDoc
            );
    ...
    const renderSeleccionModo = () => (
        <div className="flex flex-col gap-4">
            <div className="leading-tight mb-6">
                ...
                <p className="h5 text-secondary font-medium">Selecciona cómo deseas agregar el conductor</p>
            </div>
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex flex-col gap-4 justify-between">
                            <div className='flex-col'>
                                <h3 className="subtitle1 font-normal text-primary ">Completar perfil de usuario existente</h3>
                                <p className="text-secondary text-sm">
                                    Selecciona un usuario con rol conductor y completa su información de licencia
                                </p>
                            </div>

                            <p className="bg-background border border-border rounded-full text-xs shadow-2xl px-5 py-2 w-max max-w-xs">
                                {usuariosDisponibles.length} usuario{usuariosDisponibles.length !== 1 ? 's' : ''} disponible{usuariosDisponibles.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                        <md-icon className="text-secondary text-lg">call_made</md-icon>
                    </div>
        ...
        <div className="flex flex-col list-enter gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <input
                        ...
                        onChange={(e) => setNuevoUsuario({ ...nuevoUsuario, numDocumento: e.target.value })}
            ...
            <div className="flex justify-between items-center gap-2 pt-2">
                ...
                <button
                    ...
                    disabled={loading || !nuevoUsuario.tipoDoc || !nuevoUsuario.numDocumento || !nuevoUsuario.correo}
                >
                    {loading && (
    ...
    const renderStep2Usuario = () => (
        <div className="flex flex-col list-enter gap-4">
            <div className="leading-tight mb-6">
                <h2 className="h2 font-medium text-primary">Crear Conductor</h2>
                <p className="h5 text-secondary font-medium">Información personal y contacto</p>
            </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="select-wrapper">
                        ...
                        <select
                            ...
                            onChange={(e) => setNuevoUsuario({ ...nuevoUsuario, idCiudad: e.target.value })}
                            className="select-filter w-full px-4 input bg-fill border border-border rounded-lg text-primary focus:outline-none focus:border-primary transition-colors"
                            disabled={loading}
                        >
                            <option value="">Selecciona</option>
            ...
            <div className="flex justify-between items-center gap-2 pt-2">
                ...
                <button
                    ...
                    disabled={loading || !nuevoUsuario.nombre || !nuevoUsuario.telefono || !nuevoUsuario.direccion || !nuevoUsuario.idCiudad}
                >
                    Siguiente
                </button>
            </div>
    ...
    const renderSeleccionUsuarioExistente = () => (
        <div className="flex flex-col gap-4">
            <div className="leading-tight mb-6">
                ...
                <p className="h5 text-secondary font-medium">¿Cómo deseas completar la información?</p>
            </div>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-secondary">{usuario.correo} | {usuario.numDocumento}</p>
                            </div>
                            <md-icon className="text-secondary text-lg">arrow_forward</md-icon>
                        </div>
                    </button>
                ))}
            </div>
        ...
        <div className="flex flex-col gap-4">
            ...
            >
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex flex-col">
                        <h3 className="subtitle1 font-normal text-primary ">Completar datos ahora</h3>
                        <p className="text-secondary text-sm">
                            Ingresa la información de la licencia del conductor en este momento
                        </p>
                    </div>
                    <md-icon className="text-secondary text-lg">call_made</md-icon>
                </div>
    ...
    const renderFormularioLicencia = () => {
        if (mode === 'new' && completionMode === 'invite') {
            return (
                <div className="flex flex-col gap-4">
                    <div className="leading-tight mb-6">
                        <h2 className="h2 font-medium text-primary">Confirmar Invitación</h2>
                        <p className="h5 text-secondary font-medium">Revisa los detalles antes de enviar</p>
                    </div>
                    ...
                    <div className="space-y-3">
                        <div className="content-box-outline-2-small  rounded-xl p-6 w-full mb-8 flex items-center gap-4">
                            ...
                            <div className="flex-1 text-center">
                                ...
                                <p className="text-secondary text-sm max-w-56">
                                    Se le enviara un correo con los proximos pasos para completar su perfil
                                </p>
                            </div>
                        </div>
                        <div className="flex items-start gap-3 p-4 content-box-outline-9-small bg-fill rounded-xl border border-border">
                            ...
                            <div>
                                <p className="subtitle1 font-medium text-primary">Credenciales de acceso</p>
                                <p className="text-xs text-secondary">Usuario y contraseña temporal generada</p>
                            </div>
                        </div>
                        <div className="flex items-start gap-3 p-4 content-box-outline-9-small bg-fill rounded-xl border border-border">
                            <md-icon className="text-primary text-xl">assignment</md-icon>
                            <div>
                                <p className="subtitle1 font-medium text-primary">Instrucciones</p>
                                <p className="text-xs text-secondary">Indicaciones para completar sus datos de licencia</p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex flex-col gap-4">
                <div className="leading-tight mb-6">
                    ...
                    <p className="h5 text-secondary font-medium">
                        {mode === 'new' ? 'Datos de la licencia de conducir' : 'Completa la información de licencia'}
                    </p>
                </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="select-wrapper">
                        ...
                        <select
                            ...
                            onChange={(e) => setLicencia({ ...licencia, idCategoriaLicencia: e.target.value })}
                            className="select-filter w-full px-4 input bg-fill border border-border rounded-lg text-primary focus:outline-none focus:border-primary transition-colors"
                            disabled={loading}
                        >
                            <option value="">Selecciona</option>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <input
                        ...
                        onChange={(e) => setLicencia({ ...licencia, fechaVencimientoLicencia: e.target.value })}
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <textarea
                        ...
                        onChange={(e) => setLicencia({ ...licencia, observaciones: e.target.value })}
                        rows="3"
                ...
                <div className="flex justify-between items-center gap-2 pt-2 ">
                    ...
                    <button
                        onClick={mode === 'new' ? handleCrearNuevoConductor : handleCompletarPerfilExistente}
                        className="btn w-full btn-primary px-8 flex items-center justify-center gap-2"
                        disabled={loading || !licencia.idCategoriaLicencia || !licencia.fechaVencimientoLicencia}
                    >
                        {loading && (
                            <div className="inline-block animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                        )}
                        {loading ? 'Procesando...' : mode === 'new' ? 'Crear Conductor' : 'Completar Perfil'}
                    </button>
                </div>
            </div>
        );
