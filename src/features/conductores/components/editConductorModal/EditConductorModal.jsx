...
export default function EditConductorModal({ isOpen, onClose, conductor, onUpdateConductor }) {
    ...
    const [activeTab, setActiveTab] = useState('basic'); // 'basic' o 'license'
    ...
    useEffect(() => {
        if (isOpen && conductor) {
            setForm({
                ...
                numeroLicencia: conductor.numeroLicencia || conductor.usuario?.numDocumento || '',
                idCategoriaLicencia: conductor.idCategoriaLicencia || '',
                fechaVencimientoLicencia: conductor.fechaVencimientoLicencia?.split('T')[0] || '',
    ...
    useEffect(() => {
        if (!isOpen) {
            ...
            setError(null);
            setFieldErrors({});
            setActiveTab('basic'); // Reset tab cuando se cierra
        }
    }, [isOpen]);
    ...
    const validateForm = () => {
        ...
        // Validar datos del conductor (el número de licencia no se valida porque es el mismo que el documento)
        ...
        if (!form.fechaVencimientoLicencia) {
            errors.fechaVencimientoLicencia = 'La fecha de vencimiento es obligatoria';
        } else {
    ...
    const handleSubmit = async e => {
        ...
        try {
            ...
            if (!responseUsuario?.success) {
                throw new Error(responseUsuario?.message || 'Error al actualizar usuario');
            }
    ...
    const getLicenseStatus = () => {
        ...
        if (diasRestantes < 0) {
            ...
            return { tipo: 'proxima', mensaje: `Licencia próxima a vencer (${diasRestantes} días)` };
        }
        return null;
    };
                    ...
                    <div className="px-8 md:px-20">
                        ...
                        <div className="flex gap-2 mb-6 border-b border-border">
                            <button
                                ...
                                className={`px-6 py-3 font-medium transition-all relative ${activeTab === 'basic'
                                    ? 'text-primary'
                            ...
                            <button
                                ...
                                className={`px-6 py-3 font-medium transition-all relative ${activeTab === 'license'
                                    ? 'text-primary'
                                    : 'text-secondary hover:text-primary'
                                    }`}
                                disabled={loading}
                        ...
                        <form onSubmit={handleSubmit}>
                                            ...
                                            <>
                                                <div className="select-wrapper w-full">
                                                    ...
                                                    <select
                                                        ...
                                                        value={form.tipoDoc || ''}
                                                        onChange={handleChange}
                                                        className={`select-filter w-full px-4 input bg-fill border rounded-lg text-primary focus:outline-none focus:border-primary transition-colors ${fieldErrors.tipoDoc ? 'border-red' : 'border-border'
                                                            }`}
                                                        disabled={loading}
                                                    >
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <div className="select-wrapper w-full">
                                                ...
                                                <select
                                                    ...
                                                    value={form.idCiudad || ''}
                                                    onChange={handleChange}
                                                    className={`select-filter w-full px-4 input bg-fill border rounded-lg text-primary focus:outline-none focus:border-primary transition-colors ${fieldErrors.idCiudad ? 'border-red' : 'border-border'
                                                        }`}
                                                    disabled={loading}
                                                >
                            ...
                            {/* Tab de Información de Licencia */}
                            {activeTab === 'license' && (
                                <div className="flex flex-col gap-4 mb-6">
                                    {/* Datos de licencia */}
                                    <div className="flex flex-col gap-1">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <div className="select-wrapper w-full">
                                            ...
                                            <select
                                                ...
                                                onChange={handleChange}
                                                className={`select-filter w-full px-4 input bg-fill border rounded-lg text-primary focus:outline-none focus:border-primary transition-colors ${fieldErrors.idCategoriaLicencia ? 'border-red' : 'border-border'
                                                    }`}
                                                disabled={loading}
                                            >
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <input
                                            ...
                                            onChange={handleChange}
                                            className={`w-full px-4 py-3 input bg-fill border rounded-lg text-secondary focus:outline-none focus:ring-2 transition-all date-secondary ${fieldErrors.fechaVencimientoLicencia
                                                ? 'border-red focus:ring-red/20 focus:border-red'
                                                : 'border-border focus:ring-primary/20 focus:border-primary'
                                                }`}
                                            disabled={loading}
                                        ...
                                        {licenseStatus && (
                                            <div className={`flex items-center gap-2 mt-2 p-2 rounded-lg ${licenseStatus.tipo === 'vencida'
                                                ? 'bg-red/10 border border-red/30'
                                                : 'bg-yellow/10 border border-yellow/30'
                                                }`}>
                                                <md-icon className={`text-sm ${licenseStatus.tipo === 'vencida' ? 'text-red' : 'text-yellow-600'
                                                    }`}>
                                                    {licenseStatus.tipo === 'vencida' ? 'error' : 'warning'}
                                                </md-icon>
                                                <span className={`text-sm ${licenseStatus.tipo === 'vencida' ? 'text-red' : 'text-yellow-600'
                                                    }`}>
