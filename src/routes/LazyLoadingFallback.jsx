...
const LazyLoadingFallback = ({ message = "Cargando..." }) => {
    return (
        <div className="flex items-center justify-center w-full" style={{ height: 'calc(100vh - 40px)' }}>
            <div className="flex flex-col items-center justify-center" style={{ width: '340px' }}>
                <md-linear-progress indeterminate style={{ width: '100%', marginBottom: '24px' }}></md-linear-progress>
                <span className="text-secondary text-lg" style={{ textAlign: 'center' }}>{message}</span>
            </div>
        </div>
    );
};
...
const RouteLoading = ({ route }) => {
    ...
    return (
        <LazyLoadingFallback
            message={messages[route] || "Cargando página..."}
        />
    );
};
