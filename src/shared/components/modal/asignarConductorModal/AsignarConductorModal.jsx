...
try {
    conductorService = require('../../../services/conductorService').default || require('../../../services/conductorService').conductorService;
} catch (e) {
...
try {
    vehiculoService = require('../../../services/vehiculoService').default || require('../../../services/vehiculoService').vehiculoService;
} catch (e) {
...
const AsignarConductorModal = ({ 
    ...
    // Cargar datos del backend solo si no se proporcionan como props
    useEffect(() => {
        if (isOpen && (!conductoresProp || conductoresProp.length === 0 || !vehiculosProp)) {
            cargarDatos();
        } else if (isOpen && conductoresProp && vehiculosProp) {
            // Si se proporcionan datos como props, usarlos directamente
            ...
            try {
                ...
                if (Array.isArray(responseConductores)) {
                    ...
                    conductoresData = Array.isArray(responseConductores.data) 
                        ...
                        : responseConductores.data.conductores || responseConductores.data.data || [];
                    ...
                    conductoresData = Array.isArray(responseConductores.conductores) 
                        ...
                        : [];
                } else if (responseConductores?.success && responseConductores?.result) {
            ...
            try {
                ...
                if (Array.isArray(responseVehiculos)) {
                    ...
                    vehiculosData = Array.isArray(responseVehiculos.data) 
                        ...
                        : responseVehiculos.data.vehiculos || responseVehiculos.data.data || [];
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            idConductor: Yup.string().required('El conductor es requerido'),
            idVehiculo: requireVehiculo ? Yup.string().required('El vehículo es requerido') : Yup.string()
        }),
        ...
        onSubmit: async (values) => {
            try {
                ...
                const conductorSeleccionado = conductores.find(c => c.id == values.idConductor);
                const vehiculoSeleccionado = vehiculos.find(v => v.id == values.idVehiculo);
                ...
                const asignacion = {
                    ...
                    placaVehiculo: values.idVehiculo ? (vehiculoSeleccionado?.placa || 'No disponible') : null
                };
            ...
            <main className="p-6 list-enter max-h-[90vh] overflow-y-auto scrollbar-hide rounded-xl shadow-lg">
                ...
                <div className="px-4">
                    <div className="leading-tight mb-6">
                        <h2 className="h3 font-medium text-primary">{isEditMode ? 'Editar asignación conductor' : 'Asignar conductor'}</h2>
                        <div className="flex items-center gap-2 mt-3">
                            <md-icon className="text-sm text-secondary">group</md-icon>
                            <span className="text-sm text-secondary">Ver lista de pasajeros</span>
                        </div>
                    </div>
                    ...
                    <form onSubmit={formik.handleSubmit}>
                        ...
                        <div className="flex flex-col gap-2 mb-6">
                            <div className="flex items-center justify-between">
                                <label className="subtitle1 text-primary font-medium">Conductor *</label>
                            ...
                            {formik.touched.idConductor && formik.errors.idConductor && (
                                <span className="text-red text-xs">{formik.errors.idConductor}</span>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col gap-2 mb-8">
                            <div className="flex items-center justify-between">
                                <label className="subtitle1 text-primary font-medium">Vehículo *</label>
                                <span className="text-xs text-secondary">Requerido</span>
                            </div>
                            <div className="select-wrapper w-full">
                                <md-icon className="text-sm">arrow_drop_down</md-icon>
                            ...
                            {formik.touched.idVehiculo && formik.errors.idVehiculo && (
                                <span className="text-red text-xs">{formik.errors.idVehiculo}</span>
                            )}
                        </div>
                        )}
                        ...
                        <div className="flex flex-col gap-2 mb-8">
                            <div className="flex items-center justify-between">
                                <label className="subtitle1 text-primary font-medium">Vehículo (Opcional)</label>
                        ...
                        <div className="flex justify-center gap-3 mt-6">
                            ...
                            >
                                {formik.isSubmitting ? (isEditMode ? 'Actualizando...' : 'Asignando...') : (isEditMode ? 'Editar' : 'Asignar')}
                            </md-filled-button>
                        </div>
                    </form>
                </div>
