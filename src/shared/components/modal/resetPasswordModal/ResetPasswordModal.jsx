...
const ResetPasswordModal = ({ isOpen, onClose }) => {
    ...
    const { register: registerForm, handleSubmit: handleSubmitForm, formState: { errors: errorsForm }, reset } = useForm({
    ...
    const handleRestablecer = async (data) => {
        ...
        try {
            ...
            await api.post('/auth/reset-password', { code: codeValue, newPassword: data.newPassword });
    ...
    const handleCodeInput = (e, idx) => {
        ...
        // Si solo ingresa un dígito
        const digit = val.slice(0, 1);
        const newCode = [...code];
        newCode[idx] = digit;
        setCode(newCode);
        // Solo mover el foco si el valor es un número válido
        if (digit && idx < 5) {
            codeRefs[idx + 1].current?.focus();
        }
        if (!digit && idx > 0 && e.nativeEvent.inputType === 'deleteContentBackward') {
            codeRefs[idx - 1].current?.focus();
        }
    };
        ...
        <Modal isOpen={isOpen} onClose={handleClose} size='xl'>
            <div className='p-3 list-enter max-h-[90vh] overflow-y-auto scrollbar-hide rounded-xl shadow-lg'>
                ...
                <div className='w-full max-w-md mx-auto px-0 flex flex-col min-h-[500px] justify-center transition-all duration-300'>
                    {!restablecido && (
                        <div className='list-enter'>
                            <div className='flex flex-col mb-2'>
                                <h1 className='h2 font-semibold text-primary text-2xl leading-tight mb-1'>Restablecer contraseña</h1>
                                <p className='font-normal text-secondary text-sm mb-2'>Ingresa tus datos para restablecer tu contraseña.</p>
                            </div>
                    ...
                    {step === 1 && !restablecido && (
                        <form className='space-y-4 list-enter' onSubmit={handleSubmit}>
                            <div className='flex flex-col'>
                                <label htmlFor="documento" className='text-secondary subtitle1 mb-1'>Correo electrónico</label>
                                <input
                                    type="email"
                                    id="documento"
                            ...
                            <div className='pt-1'>
                                <button type="submit" className='w-full btn btn-primary py-3 font-medium text-subtitle1' disabled={enviando}>
                    ...
                    {step === 2 && !restablecido && (
                        <form className='space-y-4 list-enter' onSubmit={handleValidarCodigo}>
                            <div className='flex flex-col'>
                                <label className='text-secondary subtitle1 mb-1'>Código recibido</label>
                            ...
                            <div className='pt-1'>
                                <button type="submit" className='w-full btn btn-primary py-3 font-medium text-subtitle1' disabled={code.some(c => !c)}>
                                    Verificar
                                </button>
                            </div>
                            {errorCodigo && <div className='text-red-600 text-center'>{errorCodigo}</div>}
                        </form>
                    )}

                    {step === 3 && !restablecido && (
                        <form className='space-y-4' onSubmit={handleSubmitForm(handleRestablecer)}>
                            <div className='flex flex-col relative'>
                                <label htmlFor="newPassword" className='text-secondary subtitle1 mb-1'>Nueva contraseña</label>
                                <input
                                ...
                                {errorsForm.newPassword && <span className='text-red-500 text-sm mt-1'>{errorsForm.newPassword.message}</span>}
                            </div>
                            <div className='flex flex-col relative'>
                                <input
                                ...
                                {errorsForm.confirmPassword && <span className='text-red-500 text-sm mt-1'>{errorsForm.confirmPassword.message}</span>}
                            </div>
                            <div className='pt-1'>
                                <button type="submit" className='w-full btn btn-primary py-3 font-medium text-subtitle1' disabled={enviandoCodigo}>
                                    {enviandoCodigo ? 'Restableciendo...' : 'Restablecer'}
                                </button>
                            </div>
                            {mensajeCodigo && <div className='text-green-600 text-center'>{mensajeCodigo}</div>}
                            {errorCodigo && <div className='text-red-600 '>{errorCodigo}</div>}
                        </form>
                    )}
                    ...
                    {restablecido && (
                        <div className='flex flex-col items-center gap-2 justify-center text-center list-enter'>
                            <md-icon style={{ fontSize: '50px', height: '96px', width: '96px', display: 'block' }}>check_circle</md-icon>
                            <h1 className='h3'>¡Contraseña restablecida correctamente!</h1>
                            <p className='text-secondary'>Tu contraseña ha sido restablecida correctamente. Ahora puedes iniciar sesión utilizando tus nuevas credenciales.</p>
                            <button
