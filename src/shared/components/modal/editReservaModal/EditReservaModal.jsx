...
const reservaSchema = Yup.object().shape({
  ...
  pasajeros: Yup.array()
    .min(1, 'Debe agregar al menos un pasajero')
...
const EditReservaModal = ({ isOpen, onClose, onConfirm, reserva }) => {
      ...
      try {
        ...
        if (Array.isArray(responseClientes)) {
          ...
          clientesData = Array.isArray(responseClientes.data)
            ...
            : responseClientes.data.usuarios ||
      ...
      try {
        ...
        if (Array.isArray(responseUbicaciones)) {
          ...
          ubicacionesData = Array.isArray(responseUbicaciones.data)
            ...
            : responseUbicaciones.data.ubicaciones ||
              responseUbicaciones.data.data ||
              [];
    ...
    onSubmit: async values => {
      try {
        ...
        const updatedReserva = {
          ...
          idConductor:
            conductorAsignado?.idConductor || reserva?.idConductor || null,
          nombreConductor:
            conductorAsignado?.nombreConductor ||
            reserva?.nombreConductor ||
            null,
          idVehiculo:
            conductorAsignado?.idVehiculo || reserva?.idVehiculo || null,
          placaVehiculo:
            conductorAsignado?.placaVehiculo || reserva?.placaVehiculo || null,
        };
  ...
  const handleEliminarPasajero = index => {
    ...
    formik.setFieldValue(
      'pasajeros',
      pasajerosActuales.filter((_, i) => i !== index)
    );
  };
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-3">
                ...
                {formik.values.pasajeros &&
                formik.values.pasajeros.length > 0 ? (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-secondary font-medium mt-2">
                      Total: {formik.values.pasajeros.length} pasajero(s)
                    </p>
                  </div>
                ) : (
                  <p className="text-xs text-secondary italic">
                    Sin pasajeros agregados
                  </p>
                )}
              </div>
            ...
            <div>
              <h3 className="subtitle1 text-primary font-medium mb-4">
                Datos de la ruta
              </h3>
              <div className="flex flex-col gap-4">
                <div className="flex gap-4">
                  <div className="flex-1 flex flex-col gap-1">
                    <label className="subtitle1 text-primary font-medium text-sm">
                      Origen *
                    </label>
                    <div className="select-wrapper w-full">
                      <md-icon className="text-sm">arrow_drop_down</md-icon>
                      ...
                      >
                        ...
                        {ubicaciones && ubicaciones.length > 0 ? (
                          ubicaciones.map(ubicacion => (
                            <option
                              key={ubicacion.id}
                              value={ubicacion.nombre || ubicacion.id}
                            >
                              {ubicacion.nombre}
                            </option>
                          ))
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    ...
                    {conductorAsignado?.nombreConductor ||
                    reserva?.nombreConductor
                      ? `${conductorAsignado?.nombreConductor || reserva?.nombreConductor}`
                      : 'Asignar'}
                  </button>
                  {(conductorAsignado?.placaVehiculo ||
